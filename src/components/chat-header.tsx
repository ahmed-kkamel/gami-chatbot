...
import { ChatHeaderProps } from "../types/chat-wedgit-types";
...
export const ChatHeader = ({ handleReset, handleCloseChat, isExpanded, setIsExpanded, isMobile }: ChatHeaderProps) => {
    ...
    return (
        <div className="bg-gradient-to-r from-[#0C4A4D] to-[#083032] relative">
            ...
            <div className="flex justify-end items-center gap-2 px-4 pt-4 absolute right-0">
                {!isMobile &&
                    <button onClick={() => setIsExpanded(!isExpanded)}>
                        <Image
                            src={isExpanded && !isMobile ? "/assets/collapse.svg" : "/assets/expand.svg"}
            ...
            <div className="bg-[url(/assets/header-background.png)] bg-no-repeat bg-left bg-contain flex flex-col gap-2 items-center">
                <Image src="/assets/chat-icon.svg" alt="chat icon" width={66} height={66} className="outline outline-white rounded-full mt-3" />
                <Image src="/assets/header-icon.svg" alt="chat icon" width={24} height={24} className={`absolute ${isExpanded && !isMobile ? "end-[47.5%]" : "end-[41%]"} bottom-10`} />
                <p className={`font-semibold text-base mb-3 ${isExpanded && !isMobile ? "text-2xl" : "text-base"}`}>Welcome to GIVA</p>
            </div>
            {/* 
